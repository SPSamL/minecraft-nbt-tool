...
internal static class BlueprintWebServer
{
    ...
    private const string StyleColoniesThemeUrl = "https://github.com/ldtteam/stylecolonies/tree/release/main/src/main/resources/blueprints/stylecolonies";
    private const string MineColoniesThemeUrl = "https://github.com/ldtteam/minecolonies/tree/version/main/src/main/resources/blueprints/minecolonies";
    ...
    /// <summary>
    /// Starts the local web server and processes incoming requests.
    /// </summary>
    ...
    private static async Task HandleRequestAsync(HttpListenerContext context, string rootPath, int port)
    {
        try
        {
            ...
            var discoveredStyles = DiscoverSourceStyles(source.ScanPath);
            var sourceKey = NormalizeSourceKey(source.ScanPath);

            var combinedWarning = JoinMessages(source.Warning, cacheMessage);
            var shouldDelayDataLoadForStyleSelection = selectedStyles.Count == 0
                && (LooksLikeThemePresetUrl(repoUrl) || discoveredStyles.Count > 1);

            if (shouldDelayDataLoadForStyleSelection)
            {
                var waitWarning = JoinMessages(
                    combinedWarning,
                    "This source appears to include multiple themes. Select at least one style, then click Refresh to load blueprint data.");

                var waitingHtml = BuildHtml(
                    new BlueprintScanReport
                    {
                        RootPath = source.DisplayPath,
                        Files = [],
                        UniqueBlocksByGroup = [],
                        BlockCountsByBlueprint = []
                    },
                    [],
                    rootPath,
                    selectedStyles,
                ...
                await WriteStringAsync(context.Response, waitingHtml, "text/html; charset=utf-8");
                return;
            }
            ...
            var html = BuildHtml(report, filteredFiles, rootPath, selectedStyles, selectedBuildings, selectedLevels, localPath, repoUrl, source.DisplayPath, combinedWarning, scanStatus, port, discoveredStyles);
    ...
    private static string BuildHtml(
        ...
        string scanStatus,
        int port,
        IReadOnlyCollection<string>? discoveredStyles)
    {
        ...
        var styleOptions = (discoveredStyles ?? [])
            .Concat(report.Files.Select(file => file.Style))
            .Where(value => !string.IsNullOrWhiteSpace(value))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .OrderBy(value => value, StringComparer.OrdinalIgnoreCase)
            .ToList();

        var allStyles = styleOptions;
        ...
        builder.AppendLine($"<label>GitHub folder URL<input id=\"repo-url-input\" name=\"repoUrl\" value=\"{WebUtility.HtmlEncode(repoUrl ?? string.Empty)}\" placeholder=\"https://github.com/owner/repo/tree/branch/path\" /></label>");
        builder.AppendLine("<label>Theme URL preset<select id=\"repo-url-preset\"><option value=\"\">Custom URL...</option><option value=\"https://github.com/ldtteam/stylecolonies/tree/release/main/src/main/resources/blueprints/stylecolonies\">StyleColonies</option><option value=\"https://github.com/ldtteam/minecolonies/tree/version/main/src/main/resources/blueprints/minecolonies\">MineColonies</option></select></label>");
        ...
        builder.AppendLine("<div class=\"toolbar-actions\"><div class=\"toolbar-actions-left\"><button type=\"submit\">Refresh</button><button id=\"top-clear-filters\" type=\"button\" class=\"secondary-btn\">Clear filters</button><button type=\"submit\" name=\"clearCache\" value=\"1\" class=\"secondary-btn\">Clear cache</button></div><div class=\"toolbar-actions-right\"><button id=\"export-json-btn\" type=\"button\" class=\"secondary-btn\">Export JSON</button><button id=\"export-csv-btn\" type=\"button\" class=\"secondary-btn\">Export CSV</button></div></div>");
        ...
        builder.AppendLine("</main>");
        builder.AppendLine("<div id=\"loading-overlay\" class=\"loading-overlay\" aria-live=\"polite\" aria-hidden=\"true\"><div class=\"loading-card\"><div class=\"loading-spinner\" aria-hidden=\"true\"></div><div id=\"loading-message\" class=\"loading-message\">Loading data...</div></div></div>");
    ...
    /// <summary>
    /// Build unique minimum level rows.
    /// </summary>
    ...
    private static void AppendCountsFolder(StringBuilder builder, FolderNode node, int depth)
    {
        ...
        builder.AppendLine($"<summary class=\"folder-title\"><span class=\"folder-name\">{WebUtility.HtmlEncode(node.Name)}</span><span class=\"summary-meta\">{subtreeFiles.Count} files &middot; {subtreeDistinctBlocks} blocks</span></summary>");
        ...
        foreach (var buildingGroup in groups)
        {
            ...
            builder.AppendLine($"<summary class=\"building-title\"><span>{WebUtility.HtmlEncode(buildingGroup.Key)}</span><span class=\"summary-meta\">{buildingFiles.Count} files &middot; {buildingDistinctBlocks} blocks</span></summary>");
            ...
            foreach (var levelGroup in levelGroups)
            {
                if (hasAnyLevels && levelGroup.Key.HasValue)
                {
                    ...
                    builder.AppendLine($"<summary class=\"muted level-title\"><span>Level {WebUtility.HtmlEncode(levelGroup.Key.Value.ToString(CultureInfo.InvariantCulture))}</span><span class=\"summary-meta\">{levelFiles.Count} files &middot; {levelDistinctBlocks} blocks</span></summary>");
                }
...
input, select { width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid var(--border); background: rgba(15, 23, 42, 0.8); color: var(--text); }
...
.toolbar-actions { display: flex; gap: 8px; align-items: center; grid-column: 1 / -1; }
.toolbar-actions-left { display: flex; gap: 8px; align-items: center; }
.toolbar-actions-right { display: flex; gap: 8px; align-items: center; margin-left: auto; }
...
.loading-overlay { position: fixed; inset: 0; display: none; align-items: center; justify-content: center; background: rgba(2, 6, 23, 0.76); backdrop-filter: blur(4px); z-index: 9999; }
.loading-card { display: grid; justify-items: center; gap: 10px; min-width: 240px; padding: 18px 20px; border-radius: 14px; border: 1px solid var(--border); background: rgba(15, 23, 42, 0.95); box-shadow: 0 20px 45px rgba(0,0,0,0.35); }
.loading-spinner { width: 34px; height: 34px; border-radius: 50%; border: 3px solid rgba(148, 163, 184, 0.35); border-top-color: #bef264; animation: loading-spin 0.9s linear infinite; }
.loading-message { color: #e2e8f0; font-weight: 600; font-size: 0.92rem; }
body.is-loading { cursor: wait; }
body.is-loading .loading-overlay { display: flex; }
body.is-loading .toolbar,
body.is-loading .panel,
body.is-loading .section-toggle,
body.is-loading button,
body.is-loading input,
body.is-loading select { pointer-events: none; }
...
function beginLoadingUi(message) {
    if (window.isLoadingUiActive) {
        return;
    }

    window.isLoadingUiActive = true;

    const overlay = document.getElementById('loading-overlay');
    const loadingMessage = document.getElementById('loading-message');
    if (loadingMessage) {
        loadingMessage.textContent = message && message.trim().length > 0
            ? message.trim()
            : 'Loading data...';
    }
...
function initializeTopToolbarControls() {
    ...
    const runExport = (format) => {
        beginLoadingUi('Preparing export...');
        syncTopFilterHiddenInputs();
    ...
    const repoUrlPreset = document.getElementById('repo-url-preset');
    const updateRepoPresetSelection = () => {
        if (!repoUrlInput || !repoUrlPreset) {
            return;
        }

        const inputValue = String(repoUrlInput.value ?? '').trim();
        const matchingOption = Array.from(repoUrlPreset.options).find(option => {
            if (!option.value) {
                return false;
            }
        ...
        repoUrlPreset.value = matchingOption ? matchingOption.value : '';
    };

    if (repoUrlPreset && repoUrlInput) {
        updateRepoPresetSelection();

        repoUrlPreset.addEventListener('change', () => {
            if (!repoUrlPreset.value) {
                return;
            }

            repoUrlInput.value = repoUrlPreset.value;
            queueTopFilterSubmit();
        });

        repoUrlInput.addEventListener('input', updateRepoPresetSelection);
    }

    form.addEventListener('submit', () => {
        syncTopFilterHiddenInputs();
        beginLoadingUi('Loading blueprints...');
    });
}
...
function updateSortHeaderIndicators() {
    ...
    document.querySelectorAll('.sort-header').forEach(button => {
        ...
        indicator.textContent = sortState.direction === 1 ? 'â†‘' : 'â†“';
    });
}
    ...
    document.querySelectorAll('.requirements-table').forEach(table => {
        ...
        table.querySelectorAll('.requirements-sort-header').forEach((button, buttonIndex) => {
            button.addEventListener('click', () => {
                ...
                if (currentIndicator) {
                    currentIndicator.textContent = nextDirection === 'asc' ? 'â†‘' : 'â†“';
                }
            });
        });
    });
...
""";
    ...
    /// <summary>
    /// Checks whether the repository URL is one of the known multi-theme presets.
    /// </summary>
    private static bool LooksLikeThemePresetUrl(string? repoUrl)
    {
        ...
        return string.Equals(repoUrl.Trim(), StyleColoniesThemeUrl, StringComparison.OrdinalIgnoreCase)
            || string.Equals(repoUrl.Trim(), MineColoniesThemeUrl, StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Discovers likely style folders under each input root without parsing NBT payloads.
    /// </summary>
    private static List<string> DiscoverSourceStyles(string scanPath)
    {
        ...
        foreach (var root in roots)
        {
            ...
            {
                var candidateStyle = Path.GetFileName(childDirectory);
                if (string.IsNullOrWhiteSpace(candidateStyle))
                {
                    continue;
                }

                if (ContainsSupportedFiles(childDirectory))
                {
                    styles.Add(candidateStyle);
                }
            }
        }
    ...
    /// <summary>
    /// Checks whether a directory tree contains at least one supported blueprint file.
    /// </summary>
    private static bool ContainsSupportedFiles(string directoryPath)
    {
        try
        {
            return Directory.EnumerateFiles(directoryPath, "*.*", SearchOption.AllDirectories)
                .Any(filePath => SupportedExtensions.Contains(Path.GetExtension(filePath), StringComparer.OrdinalIgnoreCase));
        }
        catch
        {
            return false;
    ...
    /// <summary>
    /// Try get hot cached scan report.
    /// </summary>
    ...
    /// <summary>
    /// Get persistent scan cache directory.
    /// </summary>
    ...
    /// <summary>
    /// Build persistent scan cache path.
    /// </summary>
    ...
    /// <summary>
    /// Try load persistent scan report.
    /// </summary>
    ...
    /// <summary>
    /// Try clear persistent scan cache.
    /// </summary>
